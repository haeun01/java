...
-- 2. 사원 중에서 직위가 '과장' 이상인 사원들의 이름과 직위를 사원번호의 내림차순으로 조회하세요.
...
select 주문번호, 제품번호, 주문수량 from 주문세부 where 제품번호 = 77;

-- 9. 주문일이 2021년 1월 1일이전인 주문의 주문번호와 주문일을 조회하세요.
select 주문번호, 주문일 from 주문 where 주문일 < '2021-01-01';

-- 10. 사원의 성별이 여성이고, 직위가 '사원'인 사원들의 이름과 성별, 직위를 조회하세요. (AND로 사용가능)
select 이름, 성별, 직위 from 사원 where 성별 = '여' && 직위 = '사원';

-- 11. 제품의 단가가 8000원 이상인 제품들 중에서 제품명 순으로 정렬하여 조회하세요. (desc의 반대말 ASC)
select 제품명, 단가 from 제품 where 단가 >= 8000 order by 제품명;
