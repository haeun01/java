...
-- left 문자열의 왼쪽부터 n개
-- right 문자열의 오른쪽부터 n개
-- substr n번째부터 n개 (substr('SQL 공부', 5)는 5번째부터 끝까지 출력)
select left('SQL 공부', 3), right('SQL 공부', 2), substr('SQL 공부', 3, 4), substr('SQL 공부', 5);  

-- substring_index 구분자를 만날 때까지 잘라냄 
-- 인덱스는 n번째 구분자를 만날 때까지 자를지를 결정
-- 인덱스가 음수면 오른쪽부터 
...
-- rpad 오른쪽부터 채움 
select lpad('sql', 10, '#'), rpad('sql', 5, '*');  -- 주민등록번호로 자주 사용
...
-- locate 기준 문자열에서 부분 문자열의 위치 찾기
-- locate(부분문자열, 기준문자열)
select locate('world', 'hello world.'); 

-- elt 지정한 위치에 있는 문자열 반환
